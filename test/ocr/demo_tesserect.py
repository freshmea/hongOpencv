...
parser.add_argument('--model', '-m', type=str, default='data/text_recognition_CRNN_EN_2021sep.onnx',
...
def extract_text_from_box(image, box):
    ...
    # Tesseract로 텍스트 인식
    try:
        text = pytesseract.image_to_string(thresh, lang="kor+eng", config='--psm 8')
        return text.strip()
    except:
        return ""
...
if __name__ == '__main__':
    ...
    detector = PPOCRDet(modelPath='data/text_detection_en_ppocrv3_2023may.onnx',
        ...
        while cv.waitKey(1) < 0:
            ...
            if len(results[0]) and len(results[1]):
                ...
                for box, score in zip(results[0], results[1]):
                    ...
                    texts.append(
                        extract_text_from_box(original_image, box.reshape(8))
                    )
                tm.stop()
                cv.putText(frame, 'Latency - : {:.2f}'.format(tm.getFPS()), (0, 30), cv.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255))
                tm.reset()
